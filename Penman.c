...
void CalcPenman()
{
    ...
    VapourP       = fminf(Vapour[Day],SaturatedVap);
    ...
    Ea  = 0.26 * fmaxf(0.,(SaturatedVap-VapourP)) * (0.5+BU * Windspeed[Day]);
    Eac = 0.26 * fmaxf(0.,(SaturatedVap-VapourP)) * (1.0+BU * Windspeed[Day]);
   
    // Penman formula (1948)                
    // Ensure reference evaporation >= 0.   
    // Convert to cm/day                    
    Penman.E0  = fmaxf(0., 0.1 * (delta*Rnw + Gamma*Ea)/(delta + Gamma));
    Penman.ES0 = fmaxf(0., 0.1 * (delta*Rns + Gamma*Ea)/(delta + Gamma));
    Penman.ET0 = fmaxf(0., 0.1 * (delta*Rnc + Gamma*Eac)/(delta + Gamma));
